...
class DeletionService:
    ...
    def hard_delete_claims(self, db: Session) -> int:
        ...
        claims_to_delete = db.query(ClaimDocument).filter(
            and_(
        ...
        try:
            ...
            if s3_keys_to_delete:
                ...
                logger.info(f"Deleted {successful_s3_deletes}/{len(s3_keys_to_delete)} claim PDF files from S3")
    ...
    def hard_delete_line_items(self, db: Session) -> int:
        ...
        items_to_delete = db.query(ReceiptLineItem).filter(
            and_(
    ...
    def hard_delete_receipts(self, db: Session) -> int:
        ...
        receipts_to_delete = db.query(Receipt).filter(
            and_(
        ...
        try:
            ...
            if s3_keys_to_delete:
                s3_results = self.s3_service.delete_files_bulk(s3_keys_to_delete)
                successful_s3_deletes = sum(1 for v in s3_results.values() if v)
                logger.info(f"Deleted {successful_s3_deletes}/{len(s3_keys_to_delete)} receipt images from S3")
