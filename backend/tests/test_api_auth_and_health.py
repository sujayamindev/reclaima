...
def test_auth_invalid_token(monkeypatch):
    ...
    response = client.get("/api/v1/auth/me", headers={"Authorization": "Bearer fake.token.here"})
...
def test_auth_register_new_user(db_session, monkeypatch):
    from app.core.security import get_current_user
    def mock_get_current_user():
        return {"uid": "new-test-firebase-uid", "email": "new@example.com", "name": "New User"}
    app.dependency_overrides[get_current_user] = mock_get_current_user
    
    response = client.post("/api/v1/auth/register", headers={"Authorization": "Bearer valid.token.here"})
    assert response.status_code == 201
    data = response.json()
    assert data["email"] == "new@example.com"

    response2 = client.get("/api/v1/auth/me", headers={"Authorization": "Bearer valid.token.here"})
    app.dependency_overrides.pop(get_current_user, None)
