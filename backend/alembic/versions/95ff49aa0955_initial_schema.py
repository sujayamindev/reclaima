...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('return_period_days',   sa.Integer(),    nullable=True, server_default='30'),
        sa.Column('return_expiry_date',   sa.DateTime(timezone=True), nullable=True),
        sa.Column('status',
                  sa.Enum(
                      'LOCAL_ONLY', 'UPLOADED', 'PROCESSING',
                      'COMPLETED', 'OCR_FAILED', 'MANUAL_ENTRY',
                      name='receiptstatus'
                  ),
    ...
    op.create_index('ix_receipts_purchase_date',       'receipts', ['purchase_date'],       unique=False)
    op.create_index('ix_receipts_warranty_expiry_date','receipts', ['warranty_expiry_date'],unique=False)
    op.create_index('ix_receipts_return_expiry_date',  'receipts', ['return_expiry_date'],  unique=False)
    ...
    op.create_index('ix_claim_documents_receipt_id', 'claim_documents', ['receipt_id'], unique=False)
    op.create_index('ix_claim_documents_created_at', 'claim_documents', ['created_at'], unique=False)
