...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['claim_id'], ['claim_documents.id'], ondelete='CASCADE'),
    )
    
    # Create indexes for efficient querying
    op.create_index('ix_claim_defect_images_claim_id', 'claim_defect_images', ['claim_id'])
    op.create_index('ix_claim_defect_images_display_order', 'claim_defect_images', ['display_order'])
...
def downgrade():
    ...
    op.drop_index('ix_claim_defect_images_display_order', table_name='claim_defect_images')
