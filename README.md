...
![Flutter](https://img.shields.io/badge/Flutter-3.44.1-54C5F8?logo=flutter&logoColor=white)
...
│    Riverpod · Drift/SQLite cache · Firebase Auth SDK · FCM      │
...
┌──────────────────────────────┴──────────────────────────────────┐
│      FastAPI API + Scheduler  ·  OCI Compute (Docker)           │
│   Python 3.11 · SQLAlchemy ORM · Alembic · APScheduler (sep.)   │
...
Flutter authenticates with Firebase and passes JWTs to KrakenD. KrakenD validates the token using Firebase's public JWK endpoint and proxies to FastAPI — the backend port is intentionally never exposed directly. FastAPI handles all request-response logic. A separate scheduler container, running the same image, runs the APScheduler jobs: warranty and return deadline reminders plus a nightly cleanup that permanently deletes records (and their S3 objects) 30 days after soft deletion.

On the device, Drift/SQLite caches receipts so lists load instantly and stay readable offline; creating and editing receipts requires a connection.
...
| Layer | Technology |
|---|---|
| **Mobile** | Flutter 3.44.1 · Dart · Riverpod 3 · Drift/SQLite · Dio |
| **API Gateway** | KrakenD 2.6.0 |
| **Backend** | FastAPI · Python 3.11 · SQLAlchemy · Alembic · Pydantic v2 · APScheduler |
| **Database** | PostgreSQL 15 |
| **OCR & AI** | AWS Textract (AnalyzeExpense) · AWS Bedrock (Claude Haiku) |
| **Product Images** | Brave Search Image API |
| **Storage** | AWS S3 — receipt images, defect photos, and generated PDFs |
| **Auth** | Firebase Auth — email/password · Google Sign-In |
| **Notifications** | Firebase Cloud Messaging (FCM) |
| **Infrastructure** | OCI ARM Ampere VM (Always Free) · Docker Compose |
| **IaC** | Terraform — OCI network/compute, AWS IAM/S3, Infisical secrets |
| **Secrets** | Infisical |
| **CI/CD** | GitHub Actions · GHCR · Gitleaks · Trivy · Cosign |
| **Monitoring** | Prometheus · node-exporter · cAdvisor · Loki · Promtail · Grafana · Sentry |
...
Secret Scan gates all downstream jobs — if Gitleaks finds a credential in any commit being pushed or proposed, nothing else runs. The three parallel quality gates must all pass before Docker Build starts. Build and deploy only run on `main` — on a push, or on a manual run with `deploy` set to true.

Signed Android APKs are published on the [Releases](https://github.com/sujayamindev/reclaima/releases) page.
...
```bash
# Backend, KrakenD gateway, and PostgreSQL
cp backend/.env.example backend/.env    # also needs backend/firebase-service-account.json
docker-compose up -d                     # KrakenD on :8080 (backend is not exposed)
cd backend && alembic upgrade head       # set USE_MOCK_AWS=true in .env to skip AWS

# Mobile (Android emulator → local gateway)
cd mobile
flutter pub get
...
Firebase configs (`google-services.json`, `GoogleService-Info.plist`) are not in the repo and must be supplied separately. Without `API_BASE_URL`, the app points at the production API.
